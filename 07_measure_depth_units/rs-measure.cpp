...
float dist_3d(const rs2::pipeline& pipe, const rs2::depth_frame& frame, pixel u, pixel v);
...
struct toggle {
  ...
  void render_circle(const window& app, float r) {
    ...
    for (auto i = 0; i <= segments; i++) {
      ...
      glVertex2f(x * app.width() + cos(t) * r, y * app.height() + sin(t) * r);

      glVertex2f(x * app.width(), y * app.height());
    }
    glEnd();
  }
...
void render_simple_distance(const rs2::pipeline& pipe, const rs2::depth_frame& depth, const state& s,
                            const window& app);
...
int main(int argc, char* argv[]) try {
  ...
  // Decimation filter reduces the amount of data (while preserving best
  // samples)
  rs2::decimation_filter dec;
  // If the demo is too slow, make sure you run in Release
  // (-DCMAKE_BUILD_TYPE=Release) but you can also increase the following
  // parameter to decimate depth more (reducing quality)
  ...
  // Enable hole-filling
  // Hole filling is an agressive heuristic and it gets the depth wrong many
  // times However, this demo is not built to handle holes (the shortest-path
  // will always prefer to "cut" through the holes since they have zero 3D
  // distance)
  ...
  // We do this because:
  //   a. Usually depth has wider FOV, and we only really need depth for this
  //   demo b. We don't want to introduce new holes
  ...
  while (app)  // Application still alive?
  {
    ...
    if (current_frameset) {
      ...
      render_simple_distance(pipe, depth, app_state, app);
...
float dist_3d(const rs2::pipeline& pipe, const rs2::depth_frame& frame, pixel u, pixel v) {
  ...
  auto udist = frame.get_distance(upixel[1], upixel[0]);
  auto vdist = frame.get_distance(vpixel[1], vpixel[0]);

  auto distMat = depth_frame_to_meters(pipe, frame);
  auto udist1 = distMat.at<double>(upixel[0], upixel[1]);
  auto vdist1 = distMat.at<double>(vpixel[0], vpixel[1]);

  udist = udist1;
  vdist = vdist1;

  //std::cout << "udist delta = " << udist - udist1 << "\t";
  //std::cout << "vdist delta = " << vdist - vdist1 << "\n";
 
  //std::cout << distMat.rows - frame.get_height() << "\n";
  //std::cout << distMat.cols - frame.get_width() << "\n";
  // Deproject from pixel to point in 3D
  rs2_intrinsics intr = frame.get_profile()
                            .as<rs2::video_stream_profile>()
                            .get_intrinsics();  // Calibration data
  rs2_deproject_pixel_to_point(upoint, &intr, upixel, udist);
  rs2_deproject_pixel_to_point(vpoint, &intr, vpixel, vdist);

  // Calculate euclidean distance between the two points
  return sqrt(pow(upoint[0] - vpoint[0], 2) + pow(upoint[1] - vpoint[1], 2) +
...
void render_simple_distance(const rs2::pipeline& pipe, const rs2::depth_frame& depth, const state& s,
  ...
  glColor4f(0.f, 0.0f, 0.0f, 0.2f);
  draw_line(s.ruler_start.x * app.width(), s.ruler_start.y * app.height(),
            s.ruler_end.x * app.width(), s.ruler_end.y * app.height(), 9);

  glColor4f(0.f, 0.0f, 0.0f, 0.3f);
  draw_line(s.ruler_start.x * app.width(), s.ruler_start.y * app.height(),
            s.ruler_end.x * app.width(), s.ruler_end.y * app.height(), 7);

  glColor4f(1.f, 1.0f, 1.0f, 1.f);
  draw_line(s.ruler_start.x * app.width(), s.ruler_start.y * app.height(),
            s.ruler_end.x * app.width(), s.ruler_end.y * app.height(), 3);

  auto from_pixel = s.ruler_start.get_pixel(depth);
  auto to_pixel = s.ruler_end.get_pixel(depth);
  float air_dist = dist_3d(pipe, depth, from_pixel, to_pixel);
...
void register_glfw_callbacks(window& app, state& app_state) {
  app.on_left_mouse = [&](bool pressed) { app_state.mouse_down = pressed; };

  app.on_mouse_move = [&](double x, double y) {
    toggle cursor{float(x) / app.width(), float(y) / app.height()};
    std::vector<toggle*> toggles{&app_state.ruler_start, &app_state.ruler_end};
